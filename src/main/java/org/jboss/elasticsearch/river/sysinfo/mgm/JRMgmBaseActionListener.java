...
/**
 * Base action listener used for River management action calls. Handles response in case if river is not found, or if
 * there is some error.
 * 
    ...
    try {
      ...
      if (nodeInfo == null) {
        restChannel.sendResponse(new XContentRestResponse(restRequest, RestStatus.NOT_FOUND, buildMessageDocument(
            restRequest, "No SysinfoRiver found for name: " + actionRequest.getRiverName())));
      } else {
